...
func TestAgentAuthorizationBindRebindAndRevoke(t *testing.T) {
	...
	if status != http.StatusOK {
		t.Fatalf("Space B mailbox list = %d %#v", status, spaceBMailboxes)
	}
	...
	// Direct message writes remain owner-only; explicit Agent Draft operations
	// use their separately scoped versioned workflow.
