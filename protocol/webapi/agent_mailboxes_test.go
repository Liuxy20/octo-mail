...
func TestAgentMailboxCreationIsIndependent(t *testing.T) {
	...
	// Account deletion is tenant-wide. A registration in this Space must not
	// disable the same account while another Space still uses it as its active
	// gateway default.
	if _, err := s.Pool.Exec(ctx,
		`INSERT INTO gateway_identities
		 (issuer,subject,space_id,tenant_id,owner_principal_id,default_account_id)
		 VALUES ('octo-server','octo-owner-other-space','space-other',$1,$2,$3)`,
		tenantID, principalID, createdID); err != nil {
		t.Fatal(err)
	}
	status, sharedDefaultList := do(http.MethodGet, "/webapi/v0/agent-mailboxes", "", "")
	if status != http.StatusOK {
		t.Fatalf("list shared-default Agent mailbox = %d %#v", status, sharedDefaultList)
	}
	sharedDefaultFound := false
	for _, item := range sharedDefaultList["mailboxes"].([]any) {
		mailbox := item.(map[string]any)
		if mailbox["id"] == strconv.FormatInt(createdID, 10) {
			sharedDefaultFound = true
			if mailbox["deletable"] != false {
				t.Fatalf("shared gateway default is deletable: %#v", mailbox)
			}
		}
	}
	if !sharedDefaultFound {
		t.Fatalf("registered Agent mailbox missing from its Space: %#v", sharedDefaultList)
	}
	status, sharedDefaultDelete := do(http.MethodDelete, "/webapi/v0/agent-mailboxes/"+strconv.FormatInt(createdID, 10), "", "")
	if status != http.StatusConflict || sharedDefaultDelete["error"].(map[string]any)["code"] != "default_mailbox_not_deletable" {
		t.Fatalf("delete cross-Space gateway default = %d %#v", status, sharedDefaultDelete)
	}
	if _, _, _, _, err := agentDir.AuthenticateAgentCredential(ctx, credential.AccessToken); err != nil {
		t.Fatalf("failed delete revoked Agent credential: %v", err)
	}
	if _, err := s.Pool.Exec(ctx,
		`UPDATE gateway_identities SET disabled=true,updated_at=now()
		 WHERE issuer='octo-server' AND subject='octo-owner-other-space' AND space_id='space-other'`); err != nil {
		t.Fatal(err)
	}
