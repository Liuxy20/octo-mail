...
- Treat `OCTO_MAIL_S3_BUCKET` as the name of a pre-provisioned bucket. Application startup must not issue bucket-level existence probes or bucket-creation requests; deployment infrastructure owns bucket provisioning. Validate the endpoint, credentials, bucket, and object namespace with a read-only GET for an absent sentinel under the configured prefix. `NoSuchKey` and a successful GET pass; AWS's `AccessDenied` missing-object mask also passes with a warning so object-only policies do not require `ListBucket`. Missing buckets, invalid credentials/signatures, malformed responses, and transport errors fail startup.
